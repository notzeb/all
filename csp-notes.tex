...
\begin{thm}[Zhuk \cite{zhuk-strong}] An idempotent algebra $\bA$ has a Mal'cev term if and only if every binary relation $\RR \in \Inv_2(\bA)$ has the parallelogram property. More explicitly, this occurs if and only if we have
\[
...
We now prove a few elementary properties of the commutator which hold in general, which are given as exercises in Hobby and McKenzie's book \cite{hobby-mckenzie}.
...
%\subsection{Classification of three element minimal Taylor algebras}
